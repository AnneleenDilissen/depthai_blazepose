...
xout_Edge = pipeline.create(depthai.node.XLinkOut)
xout_Edge.setStreamName('edge')
cam_rgb.video.link(edgeDetector.inputImage)
...
with depthai.Device(pipeline, usb2Mode=True) as device:
    ...
    edge = device.getOutputQueue("edge", 8, False)
    ...
    while True:
        ...
        if frame is not None:
            ...
            cv2.imshow("edge", edgeRgbQueue.getCvFrame())

...
class BlazeposeDepthai:
    ...
    def __init__(self, input_src="rgb",
        ...
        # Create a new dai.Device object with the given configuration options and set the context for the device
        self.device = dai.Device(openvino_version, device_info, True)
        ...
        if input_src == None or input_src == "rgb" or input_src == "rgb_laconic":
            ...
            if self.crop:
                ...
                self.crop_w = (int(round(1280 * self.scale_nd[0] / self.scale_nd[1])) - self.img_w) // 2

            else:
                width, self.scale_nd = mpu.find_isp_scale_params(internal_frame_height * 1280 / 720, is_height=False)
                self.img_h = int(round(720 * self.scale_nd[0] / self.scale_nd[1]))
                self.img_w = int(round(1280 * self.scale_nd[0] / self.scale_nd[1]))
    ...
    def next_frame(self):
        ...
        if res["type"] != 0 and res["lm_score"] > self.lm_score_thresh:
            ...
            body.rect_points = mpu.rotated_rect_to_points(body.rect_x_center_a, body.rect_y_center_a, body.rect_w_a,
    ...
    def exit(self):
        ...
        if self.stats:
            ...
            print(
                f"# landmark inferences       : {self.nb_lm_inferences} - # after pose detection: {self.nb_lm_inferences - self.nb_lm_inferences_after_landmarks_ROI} - # after landmarks ROI prediction: {self.nb_lm_inferences_after_landmarks_ROI}")
